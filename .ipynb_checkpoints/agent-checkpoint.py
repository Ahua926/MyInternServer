...
class ChatHistory():
    def __init__(self, initial_prompt=None):
        ...
        self.log_path = '{folder}/{curtime}.log'.format(folder=today,curtime=timenow)
...
class my_agent():
    def __init__(self, initial_prompt=None):
        ...
        self.history = ChatHistory(initial_prompt)
        
        
    def load_model(self, model_dir='Shanghai_AI_Laboratory/internlm2_5-7b-chat'):
    ...
    def prepare_generation_config(self, max_length=32768, top_p=0.8, temperature=0.8):
    ...
    def chat_once(self, prompt):
        real_prompt = self.history.combine_history(prompt)

        self.history.update('user', prompt)
        ...
        self.history.update('robot', cur_response)
